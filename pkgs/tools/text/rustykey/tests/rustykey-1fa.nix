...
# Run this test as:
#
#     nix-build  ./pkgs/tools/text/rustykey/tests/rustykey.nix -A driverInteractive && ./result/bin/nixos-test-driver --no-interactive
#
...
#
# This test assumes a yubikey is plugged in and is configured for chal-reponse without touch on slot 1
# it is testing rustykey format 1fA correctly setting up for rustykey in the initrd to unlock the device
...
#      boot.initrd.secrets."/etc/cryptroot.key" = keyfile;
  ...
  testScript = ''
    ...
    machine.succeed("echo -n supersecret |  rustykey format --cryptsetup cryptsetup --salt-iter-file /crypt-storage/crypt-storage/default --yubikey-slot 1 --1FA --device /dev/vdb --password-file=- -- --iter-time=1")
    # todo use bellow command when rustykey doesnt require useless password file in 1fa mode
    # machine.succeed("echo -n supersecret |  rustykey format --cryptsetup cryptsetup --salt-iter-file /crypt-storage/crypt-storage/default --yubikey-slot 1 --1FA --device /dev/vdb -- --iter-time=1")
